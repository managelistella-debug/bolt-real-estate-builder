...
export default async function AdminDashboardPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-white/70">Manage your Aspen website content.</p>

      <div className="mt-8 grid gap-4 md:grid-cols-3">
        {cards.map((card) => (
          <div key={card.label} className="rounded-lg border border-white/10 bg-[#0b3a30] p-5">
            <p className="text-sm text-white/60">{card.label}</p>
            <p className="mt-2 text-3xl text-white">{card.value}</p>
          </div>
        ))}
      </div>
    </div>

...
function getConfigForBreakpoint(config: ListingFeedConfig, bp: Breakpoint): ListingFeedConfig {
  ...
  return { ...config, columns: cols as 1 | 2 | 3, imageHeight: ih };
}
...
export function ListingFeedPreview({ config, listings }: ListingFeedPreviewProps) {
    ...
    <div>
      ...
      <div className="mb-4 flex items-center gap-1">
        {(Object.keys(BP_ICONS) as Breakpoint[]).map((bp) => {
          ...
          return (
            <button key={bp} type="button" onClick={() => setBreakpoint(bp)} className={`flex h-[30px] items-center gap-1.5 rounded-lg px-2.5 text-[12px] capitalize transition-colors ${breakpoint === bp ? 'bg-black text-white' : 'border border-[#EBEBEB] bg-white text-[#888C99] hover:bg-[#F5F5F3] hover:text-black'}`}>
      ...
      {/* Pagination */}
      {effectiveConfig.paginationType === 'pagination' && effectiveConfig.itemsPerPage !== 'unlimited' && totalPages > 1 && (
      ...
      {/* Load More */}
      {effectiveConfig.paginationType === 'load_more' && hasMore && (

...
import { Plus, Edit, Copy, Trash2, Home, MoreVertical } from 'lucide-react';
...
function PageRow({ page, websiteId, onEdit, onDuplicate, onDelete, onSetHomepage }: PageRowProps) {
  return (
    <div className="flex items-center justify-between p-4 border-b last:border-0 hover:bg-muted/50 transition-colors">
      ...
      <div className="flex items-center gap-2">
        <Link href={`/sites/${websiteId}/builder?pageId=${page.id}`}>
...
export default function SitePagesPage({ params }: { params: Promise<{ siteId: string }> }) {
  ...
  const { websites, updatePage, duplicatePage, deletePage, setHomepage } = useWebsiteStore();
  ...
  const handleCreatePage = () => {
    ...
    useWebsiteStore.setState((state) => ({
      websites: state.websites.map(w =>
        w.id === siteId
          ? { ...w, pages: [...w.pages, newPage], updatedAt: new Date() }
          : w
      ),
    }));
            ...
            {website.pages.length === 0 ? (
              ...
              website.pages.map((page) => (
                <PageRow
                  ...
                  page={page}
                  websiteId={siteId}
                  onEdit={() => router.push(`/sites/${siteId}/builder?pageId=${page.id}`)}
